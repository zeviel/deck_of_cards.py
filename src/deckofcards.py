...
class DeckOfCards:
	...
	def shuffle_cards(self, deck_count: int = 1):
		return get(
			f"{self.api}/deck/new/shuffle/?deck_count={deck_count}",
			headers=self.headers).json()
	
	def draw_card(self, deck_id: str = "new", count: int = 1):
		return get(
			f"{self.api}/deck/{deck_id}/draw/?count={count}",
			headers=self.headers).json()
	
	def reshuffle_cards(self, deck_id: str, remaining: bool = False):
		url = f"{self.api}/deck/{deck_id}/shuffle"
		if remaining:
			url += f"?remaining={remaining}"
		return get(url, headers=self.headers).json()
	
	def brand_new_deck(self, jokers_enabled: bool = False):
		return get(
			f"{self.api}/deck/new?jokers_enabled={jokers_enabled}",
			headers=self.headers).json()
	
	def partial_deck(self, cards: str = "AS,2S,KS,AD,2D,KD,AC,2C,KC,AH,2H,KH"):
		return get(
	...
	def shuffle_piles(self, deck_id: str, pile_name: str):
		return get(
			f"{self.api}/deck/{deck_id}/pile/{pile_name}/shuffle",
			headers=self.headers).json()
	
	def listing_cards_in_piles(self, deck_id: str, pile_name: str):
		return get(
	...
	def draw_bottom(self, deck_id: str):
		return get(
			f"{self.api}/deck/{deck_id}/draw/bottom",
			headers=self.headers).json()
	
	def draw_random(self, deck_id: str):
		return get(
			f"{self.api}/deck/{deck_id}/draw/random",
			headers=self.headers).json()
	
	def return_cards(self, deck_id: str):
		return get(
			f"{self.api}/deck/{deck_id}/return", headers=self.headers).json()
